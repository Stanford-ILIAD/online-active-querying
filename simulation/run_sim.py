...
def evaluate(agent, env, idx, n_episodes=10, expert_policy=None):
    """Deep Q-Learning.
    """
    scores = []
    nums_queries = []
    ...
    for i_episode in range(1, n_episodes+1):
        ...
        for t in range(500):
            ...
            if done:
                break
        p_task.append(np.nan_to_num(task_dist[idx] / np.sum(task_dist)))
        scores.append(score)
...
def run_evaluation(seed, param):
    ...
    print(task_idx, eval_envs[task_idx], policy_returns[task_idx])
    if args.env == 'peg':
        env = make_env.make_peg(*eval_envs[task_idx])
    elif args.env == 'door':
    ...
    return evaluate(agent, env=env, idx=task_idx, n_episodes=args.nval, expert_policy=partial(expert_policy, Q=get_policy_q(policy_params))) 
...
for i, loc in tqdm.tqdm(list(enumerate(x for x in policy_dirs if x.startswith(args.env)))):
    policy_params = f'{args.policies}/{loc}'
    if 'params.pkl' not in os.listdir(policy_params):
        continue
    final_returns = pd.read_csv(f'{policy_params}/progress.csv')['eval/Returns Mean'].iloc[-1]
    if final_returns < 100:
        continue
    get_policy_q(policy_params)
    env = [float(x) for x in re.findall(r'-?([0-9]+[.][0-9]+)+', loc)]
    data = torch.load(f'{policy_params}/params.pkl')
    qf1 = data['trainer/qf1']
    if [x.shape for x in qf1.parameters()][0][1] not in [39, 40]:
        continue
    if len(train_policies) < args.npol:
        train_policies.append(policy_params)
        policy_returns.append(final_returns)
        eval_policies.append(policy_params)
        eval_envs.append(env)

print(f'{len(train_policies) + len(eval_policies)} policies found')
...
if __name__ == "__main__":
    ...
    for itr in range(args.niter):
        ...
        queries, scores, probs = zip(*starmap(run_evaluation, [(seed, x) for x in params]))
        frontiers.append((np.array(queries), np.array(scores), np.array(probs)))
